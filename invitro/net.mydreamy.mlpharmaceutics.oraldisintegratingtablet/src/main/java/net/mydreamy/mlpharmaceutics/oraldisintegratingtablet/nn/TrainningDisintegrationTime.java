...
//        .allowCrossDeviceAccess(true);
        ...
        try {
        	recordReadertrain.initialize(new FileSplit(new ClassPathResource("nn/trainingset.csv").getFile()));
...
//        log.info("training set:" + trainningData.getLabels().toString());
        ...
        try {
        	recordReadertest.initialize(new FileSplit(new ClassPathResource("nn/testingset.csv").getFile()));
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
...
//        log.info("training set R is:" + String.format("%.4f", evalTrain.correlationR2(0)));
        ...
        EvaluationMCloud.AccuracyMAE(lablesTrain, PredictionTrain, 0.10);
