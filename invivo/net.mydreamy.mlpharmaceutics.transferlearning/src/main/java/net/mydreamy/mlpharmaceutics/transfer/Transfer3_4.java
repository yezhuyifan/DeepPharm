...
//	            .addLayer("MOUT2", new DenseLayer.Builder().activation(Activation.TANH).nIn(100).nOut(1000).build(), "FEATURE")
	            .addLayer("HIDDEN", new DenseLayer.Builder().activation(Activation.TANH).nIn(100).nOut(100).build(), "FEATURE")

		        .addLayer("Bioavailability", new OutputLayer.Builder().activation(Activation.SIGMOID)
                .lossFunction(new WeightedL1LossADME(3))
                .nIn(100).nOut(1).build(), "HIDDEN")
...
//			}
	      ...
	      File locationToSave = new File("Tranfer3-4.zip");       //Where to save the network. Note: the file is in .zip format - can be opened externally
	        boolean saveUpdater = true;                                             //Updater: i.e., the state for Momentum, RMSProp, Adagrad etc. Save this if you want to train your network more in the future
	        try {
				ModelSerializer.writeModel(ADMEnet, locationToSave, saveUpdater);
